            ...
            <ul class="product-value-anything product-value-price hide" style="display: block;">
              ...
              <li class="nett fn-clear">
                <?php if($goods['promoting'] == 1): ?><span class="attr ssm-price-label">优&nbsp;惠&nbsp;价&nbsp;</span>
                  <?php else: ?>
                  <span class="attr fn-price-label">商城&nbsp;价&nbsp;</span><?php endif; ?>
...
<div class="product-side fl"> 
  ...
  <div class="side-public side-look sale-top">
    ...
    <ul>
      <?php
 $md5_key = md5("select * from `__PREFIX__goods` where is_recommend = 1 and cat_id=$goods[cat_id] order by goods_id desc limit 6"); $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__goods` where is_recommend = 1 and cat_id=$goods[cat_id] order by goods_id desc limit 6"); S("sql_".$md5_key,$sql_result_v,TPSHOP_CACHE_TIME); } foreach($sql_result_v as $k=>$v): ?><li>
      ...
      <div class="fn-mc fix">
        ...
        <div class="fn-mc-rt">
          ...
          <p class="double_points">
            <span>发表评价可获得TPshop积分哦~ <br> 加精置顶还可赢双倍积分<br></span>
           	 详见<a href="">[积分规则]</a></p>
        </div>
      </div>
      <!-- /.fn-mc --> 
