...
<div class="clearfix tp_page" id="wrapper">
          ...
          <div class="u-ms-lst slide fl oh" id="slideMain">
            <ul class="slider-main">
              <?php $pid ='50'.$_GET[id].'1';$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->getField("position_id,position_name,ad_width,ad_height");$result = D("ad")->where("pid=$pid  and enabled = 1 and start_time < 1477105200 and end_time > 1477105200 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("6")->select(); if(!in_array($pid,array_keys($ad_position)) && $pid) { M("ad_position")->add(array( "position_id"=>$pid, "position_name"=>CONTROLLER_NAME."页面自动增加广告位 $pid ", "is_open"=>1, "position_desc"=>CONTROLLER_NAME."页面", )); delFile(RUNTIME_PATH); } $c = 6- count($result); if($c > 0 && $_GET[edit_ad]) { for($i = 0; $i < $c; $i++) { $result[] = array( "ad_code" => "/Public/images/not_adv.jpg", "ad_link" => "/index.php?m=Admin&c=Ad&a=ad&pid=$pid", "title" =>"暂无广告图片", "not_adv" => 1, "target" => 0, ); } } foreach($result as $k=>$v): $v[position] = $ad_position[$v[pid]]; if($_GET[edit_ad] && $v[not_adv] == 0 ) { $v[style] = "filter:alpha(opacity=50); -moz-opacity:0.5; -khtml-opacity: 0.5; opacity: 0.5"; $v[ad_link] = "/index.php?m=Admin&c=Ad&a=ad&act=edit&ad_id=$v[ad_id]"; $v[title] = $ad_position[$v[pid]][position_name]."===".$v[ad_name]; $v[target] = 0; } ?><li data-bgcolor="<?php echo ($v["bgcolor"]); ?>" class="<?php if($k == 0): ?>z-select<?php endif; ?>" style="position: absolute; z-index: 0; opacity: 0;"> 
              	<a href="<?php echo ($v["ad_link"]); ?>" <?php if($v['target'] == 1): ?>target="_blank"<?php endif; ?>><img src="<?php echo ($v[ad_code]); ?>" alt=""  title="<?php echo ($v[title]); ?>" style="<?php echo ($v[style]); ?>"></a> </li><?php endforeach; ?>
            </ul>
            <ul class="s-ctr slider-nav" style="width: 1000px;">
              <?php $pid ='50'.$_GET[id].'1';$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->getField("position_id,position_name,ad_width,ad_height");$result = D("ad")->where("pid=$pid  and enabled = 1 and start_time < 1477105200 and end_time > 1477105200 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("6")->select(); if(!in_array($pid,array_keys($ad_position)) && $pid) { M("ad_position")->add(array( "position_id"=>$pid, "position_name"=>CONTROLLER_NAME."页面自动增加广告位 $pid ", "is_open"=>1, "position_desc"=>CONTROLLER_NAME."页面", )); delFile(RUNTIME_PATH); } $c = 6- count($result); if($c > 0 && $_GET[edit_ad]) { for($i = 0; $i < $c; $i++) { $result[] = array( "ad_code" => "/Public/images/not_adv.jpg", "ad_link" => "/index.php?m=Admin&c=Ad&a=ad&pid=$pid", "title" =>"暂无广告图片", "not_adv" => 1, "target" => 0, ); } } foreach($result as $k=>$v): $v[position] = $ad_position[$v[pid]]; if($_GET[edit_ad] && $v[not_adv] == 0 ) { $v[style] = "filter:alpha(opacity=50); -moz-opacity:0.5; -khtml-opacity: 0.5; opacity: 0.5"; $v[ad_link] = "/index.php?m=Admin&c=Ad&a=ad&act=edit&ad_id=$v[ad_id]"; $v[title] = $ad_position[$v[pid]][position_name]."===".$v[ad_name]; $v[target] = 0; } ?><li class="slider-item <?php if($k == 0): ?>z-select<?php endif; ?>"></li><?php endforeach; ?>
            </ul>
            <div class="s-pg J_page_box" style="display: none;">
            	<a href="javascript:;" class="s-prev slider-prev">&lt;</a>
            	<a href="javascript:;" class="s-next slider-next" style="margin-left: 440px;">&gt;</a>
            </div>
          </div>
        </div>
      </div>
  ...
  <?php if(is_array($channel_cate)): foreach($channel_cate as $k=>$vo): if($k < 3): ?><div class="module F_dynamic" parentmoduleid="<?php echo ($k); ?>">
    <div class="layout beauty-floor beauty-floor-01 container mt30" data-floor="1">
      ...
      <?php  $cat_id_arr = getCatGrandson($vo[id]); $cat_id_str = implode(',',$cat_id_arr); ?>
      ...
      <div class="brand-slide editable ui-switchable" moduleid="1557937_bottom" fnp="brand-slide" data-widget-cid="widget-626a691d3389b7c9fc8bb8496ad82b83">
        ...
        <?php
 $md5_key = md5("select * from `__PREFIX__goods` where cat_id in($cat_id_str) and is_on_sale = 1 order by goods_id limit 4"); $sql_result_v5 = S("sql_".$md5_key); if(empty($sql_result_v5)) { $Model = new \Think\Model(); $result_name = $sql_result_v5 = $Model->query("select * from `__PREFIX__goods` where cat_id in($cat_id_str) and is_on_sale = 1 order by goods_id limit 4"); S("sql_".$md5_key,$sql_result_v5,TPSHOP_CACHE_TIME); } foreach($sql_result_v5 as $k5=>$v5): ?><li class="item ui-switchable-clone" style="float: left;"> <a href="javascript:;"> <img class="img img100" src="<?php echo (goods_thum_images($v5["goods_id"],150,150)); ?>"/> </a>
